...
def obtenir_reponse(historique: list, prenom: str, profil: dict, agent_gender: str = "feminine") -> str:
    # Update SYSTEM_PROMPT_BASE dynamically for Streamlit too
    persona = "une assistante" if agent_gender == "feminine" else "un assistant"
    relation = "une grande sœur bienveillante" if agent_gender == "feminine" else "un grand frère bienveillant"
    light = "une lumière douce" if agent_gender == "feminine" else "un guide bienveillant"
    
    # We need to define or reference the base prompt here. Since it's a global string, we can format it.
    prompt_system = SYSTEM_PROMPT_BASE.replace("une assistante", persona).replace("une grande sœur bienveillante", relation).replace("une lumière douce", light)
    
    if profil:
        memoire = f"\n\n## CE QUE TU SAIS SUR {prenom.upper()}...\n" # ... simplification
        prompt_system += memoire
    else:
        prompt_system += f"\n\nL'étudiant(e) s'appelle {prenom}."
...
def afficher_exercices():
    ...
    if not st.session_state.ex_actif:
        ...
        if ei < len(etapes):
            ...
            # Sécurité : on s'assure que la valeur reste entre 0.0 et 1.0
...
if st.session_state.user_id is None:
    ...
    with col2:
        ...
        pin_input = st.text_input("", placeholder="Code secret (5 chiffres)...", type="password", max_chars=5, label_visibility="collapsed")
        genre_agent = st.selectbox("Genre d'Aura :", ["👩 Féminin", "👨 Masculin"])
        ...
        if st.button("✨ Commencer avec Aura", use_container_width=True):
            ...
            pin_nettoye = pin_input.strip()
            if not valider_prenom(prenom_nettoye):
                st.error("Prénom invalide. Utilise uniquement des lettres (2-50 caractères).")
            elif not pin_nettoye.isdigit() or len(pin_nettoye) != 5:
                st.error("Code secret invalide. Utilise 5 chiffres.")
            else:
                try:
                    uid = obtenir_ou_creer_id_anonyme(prenom_nettoye, pin_nettoye)
                    ...
                    st.session_state.agent_gender = "feminine" if "Féminin" in genre_agent else "masculine"
